...
class ThreeET_Eyetracking(Dataset):
    ...
    def __init__(
        ...
        # data_dir = os.path.join(save_to, "ThreeET_h5")
        data_dir = save_to
        ...
        self.data = [os.path.join(data_dir, "data", f, f + ".h5") for f in filenames]
        # self.data = [os.path.join(data_dir, "data", f + ".h5") for f in filenames]
        self.targets = [os.path.join(data_dir, "data", f, "label.txt") for f in filenames]
    ...
    def __getitem__(self, index: int) -> Tuple[Any, Any]:
        ...
        with open(self.targets[index], "r") as f:
            ...
            target = np.array(
                [list(map(int, line.strip('()\n').split(', '))) for line in f.readlines()], np.float64
            )

        # event =[]
        # for i in range(len(events)):
        #     event.append(np.array(tuple(events[i])))
...
Dataset  = ThreeET_Eyetracking(save_to= "/users/cgao1/ssd/DATA/threeET_h5", split="train")
